...
const Logo = ({ logoWrapperStyle, logoStyle, titleStyle, withAnchor, anchorProps, logoSrc, title, ...props }) => (
  <Link {...props} {...logoWrapperStyle}>
    {withAnchor ? (
      <a {...anchorProps}>{logoSrc ? <Image width="126" src={logoSrc.src} alt={title} {...logoStyle} /> : <Text content={title} {...titleStyle} />}</a>
    ) : (
      <>{logoSrc ? <Image width="126" src={logoSrc.src} alt={title} {...logoStyle} /> : <Text content={title} {...titleStyle} />}</>
    )}
  </Link>
);
